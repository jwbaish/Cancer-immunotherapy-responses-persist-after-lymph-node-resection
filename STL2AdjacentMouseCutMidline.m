...
for i=1:nFacets
   % Make a cut along the entire midline  
   % low z values
     Low = any(Vskin(Fskin(i,:),3)<50);
     % Is there a sign change in x from point 1 to 2
     Across = (Vskin(Fskin(i,1),1).*Vskin(Fskin(i,2),1))<0;
     Across = Across | (Vskin(Fskin(i,2),1).*Vskin(Fskin(i,3),1))<0;
     Across = Across | (Vskin(Fskin(i,3),1).*Vskin(Fskin(i,1),1))<0;
     Cut = Low & Across;   
  if ~Cut
